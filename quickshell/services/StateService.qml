...
Singleton {
    ...
    property bool isSaving: false  // Éviter les boucles infinies

    Component.onCompleted: loadState()

    // Surveiller les modifications externes du fichier state.json
    FileView {
        id: stateWatcher
        path: root.statePath
        watchChanges: true
        ...
        onFileChanged: {
            // Ne pas recharger si c'est nous qui avons sauvegardé
            if (!root.isSaving) {
                console.log("[StateService] External change detected, reloading state");
                root.loadState();
            }
        }
    }
