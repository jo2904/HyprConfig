...
Singleton {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (internal.gpuType === "nvidia") {
                ...
                updateIntelGpuUsage.running = true;
    ...
    Process {
        ...
        command: ["bash", "-c", `
            if command -v nvidia-smi &>/dev/null && nvidia-smi &>/dev/null; then
                echo "nvidia"
            elif ls /sys/class/drm/card*/device/gpu_busy_percent 2>/dev/null | head -1 | grep -q .; then
                echo "amd"
            elif lspci | grep -qi 'vga.*intel' || [ -d /sys/class/drm/card0/gt ] || [ -d /sys/class/drm/card1/gt ]; then
                echo "intel"
            else
                echo "unknown"
            fi
    ...
    Process {
        id: updateIntelGpuUsage
        command: ["bash", "-c", `
            # Estimate Intel GPU usage based on frequency (like btop does)
            for card in /sys/class/drm/card*/; do
                if [ -f "\${card}gt_cur_freq_mhz" ] && [ -f "\${card}gt_max_freq_mhz" ]; then
                    cur=$(cat "\${card}gt_cur_freq_mhz" 2>/dev/null)
                    min=$(cat "\${card}gt_min_freq_mhz" 2>/dev/null || echo "300")
                    max=$(cat "\${card}gt_max_freq_mhz" 2>/dev/null)
                    if [ -n "$cur" ] && [ -n "$max" ] && [ "$max" -gt "$min" ]; then
                        echo "scale=0; ($cur - $min) * 100 / ($max - $min)" | bc
                        exit 0
                    fi
                fi
            done
        ...
        stdout: SplitParser {
            onRead: data => {
                const usage = parseInt(data.trim());
                if (!isNaN(usage)) {
                    internal.gpuUsage = Math.max(0, Math.min(100, usage));
                }
            }
        }
    }
