    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            QuickSettingsTile {
                ...
                icon: NetworkService.ethernetIcon
                label: "Ethernet"
                subLabel: NetworkService.ethernetStatusText
                active: NetworkService.ethernetConnected
                hasDetails: true
                onToggled: {
                    // Toggle first ethernet device
                    if (NetworkService.ethernetDevices.length > 0) {
                        const dev = NetworkService.ethernetDevices[0];
                        if (dev.state === "connected") {
                            NetworkService.disconnectEthernet(dev.device);
                        } else if (dev.state === "disconnected") {
                            NetworkService.connectEthernet(dev.device);
                        }
                    }
                }
                onOpenDetails: pageStack.currentIndex = 6
            }
